    ...
    for n in range(iter):
        ...
        for (i, j), value_ij in np.ndenumerate(heightmap):
            ...
            if east_exists and south_exists:
                ...
                square_top = heightmap[i, j] + heightmap[i, j + 1] + diamond_center
                if north_exists:
                ...
                square_left = heightmap[i, j] + heightmap[i + 1, j] + diamond_center
                if west_exists:
...
def heightmap_radar_list(heightmap, r_step, theta_step, init_angle=0, sweep=np.pi):
